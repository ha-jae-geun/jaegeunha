# 설계
```java
문 4. <보기>는 모듈화를 중심으로 한 소프트웨어 설계방
법에 대한 설명이다. 빈칸의 내용을 올바르게 나열한 것
은?

<보기>

◦ 결합도(coupling)와 응집도(cohesion)는 모듈의
( ᄀ )을 판단하는 기준이다.
◦ 결합도란 모듈 ( ᄂ )의 관련성을 의미하며, 응집도
란 모듈 ( ᄃ )의 관련성을 의미한다.
◦ 좋은 설계를 위해서는 결합도는 ( ᄅ ), 응집도는
( ᄆ ) 방향으로 설계해야 한다.
ᄀ ᄂ ᄃ ᄅ ᄆ
1 독립성 사이 내부 작게 큰
2 독립성 내부 사이 크게 작은
...
답 1
▷ 응집도(cohesion)는 모듈 내부의 기능들이 관련되어 있는 정도,
결합도(coupling)는 모듈과 모듈 간의 상호 의존 정도를 나타낸
것이다. 응집도는 높을수록, 결합도는 낮을수록 좋다.
ᄀ 독립성
ᄂ 사이
ᄃ 내부
ᄅ 작게
