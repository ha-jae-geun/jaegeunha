# 문제
```java
문 10. 인터럽트 처리를 위한 <보기>의 작업이 올바로 나
열된 것은?

<보기>
ᄀ. 인터럽트 서비스 루틴을 수행한다.
ᄂ. 보관한 프로그램 상태를 복구한다.
ᄃ. 현재 수행 중인 명령을 완료하고 상태를 저장한
다.
ᄅ. 인터럽트 발생 원인을 찾는다.
1 ᄃ → ᄅ → ᄀ → ᄂ
2 ᄃ → ᄅ → ᄂ → ᄀ
...
✠ 인터럽트 처리 과정
1. 인터럽트 요청 신호 발생
2. 현재 수행중인 명령을 완료하고 상태를 저장 -- (ᄃ)
3. 어느 장치가 인터럽트를 요청하였는지 확인 --- (ᄅ)
4. 인터럽트 서비스(취급) 루틴을 수행 ---------- (ᄀ)
5. 보존한 프로그램 상태를 복귀 --------------- (ᄂ)



문 7. 데이지-체인(daisy-chain) 우선순위 인터럽트 방식
에 대한 설명으로 옳은 것은?
1 인터럽트를 발생시키는 장치들이 병렬로 연결된다.
2 두 개 이상의 장치에서 동시에 인터럽트가 발생되면
중앙처리장치(CPU)는 이들 인터럽트를 모두 무시한
다.
3 인터럽트를 발생시킨 장치가 인터럽트 인식
(acknowledge) 신호를 받으면 자신의 장치번호를
중앙처리장치로 보낸다.
4 중앙처리장치에서 전송되는 인터럽트 인식 신호는
우선순위가 낮은 장치부터 높은 장치로 순차적으로
전달된다.

답 3
3 인터럽트가 발생하면 CPU는 어디서 인터럽트가 발생했는지 확
인하기 위해 각 장치들로 인터럽트 인식 신호를 보내는데, 그 때
인터럽트가 발생한 해당 장치는 자신이 인터럽트를 발생시켰다는
의미로 자신의 장치번호를 CPU로 보낸다.
<오답 체크> 1 데이지-체인은 직렬 방식이다.
2 두 개의 장치에서 동시에 인터럽트가 발생하면 우선순위에 따라
처리한다.
4 인터럽트 인식 신호는 우선순위가 높은 장치부터 전달된다.
◈ 인터럽트 우선순위 처리 방법
▷ 소프트웨어적인 판별 방법: 폴링(Polling)
▷ 하드웨어적인 판별 방법: 벡터 인터럽트(Vector Interrupt)
- 직렬 방식 : 데이지 체인(Daisy-chain)
- 병렬 방식
