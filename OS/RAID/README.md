# 문제
```java
31. RAID-5는 RAID-4의 어떤 문제점을 보완하기 위하여 개발되
었는가?  3번
① 병렬 액세스의 불가능
② 긴 쓰기 동작 시간
③ 패리티 디스크의 액세스 집중
...
RAID에 대한 설명으로 옳은 것은? 4번
① RAID 레벨 1은 패리티를 이용한다.
② RAID 레벨 0은 디스크 미러링을 이용한다.
③ RAID 레벨 0과 RAID 레벨 1을 조합해서 사용할 수 없다.
④ RAID 레벨 5는 패리티를 모든 디스크에 분산시킨다.
