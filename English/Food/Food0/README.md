...
## 밀
* 이에 비해 밀은 좀 춥고 건조한 기후에서 잘 자라는 특성을 지녀 유럽에서 많이 재배됐다. 그런데 밀농사는 자연 강우에만 의존하면 되고 혼자서도 관리가 가능해 그만큼 다른 사람과의 관계를 중시할 필요가 없다. 밀은 지력을 많이 소모하는 작물이어서 휴경이 필수다. 따라서 매년 쌀을 생산할 수 있는 동양과 달리 서양에서는 대규모로 식량을 거래할 수 있는 상업 위주의 문명이 발달했다. 개인주의적이고 논리적인 성향이 강하며, 동양처럼 왕권국가가 아닌 도시국가가 발달한 이유에 대한 설명이다. 이처럼 쌀과 밀로써 동서양 간의 문화적 차이를 설명하는 대표적인 학자 중 한 명이 미국 시카고대학의 행동학자 토머스 탈헬름이다. 그는 지난 2014년에 발표한 연구 결과를 통해 자신의 가설을 증명해보였다.

# 감자
* 실온에 놓으면 온도가 높아서 싹이 나고 냉장고에 넣으면 습기가 높아서 싹이 난다.

# 토마토
* 토마토를 냉장고에 넣으면 세포막이 손상되면서 수분도 날아가면서 맛없어진다.

# 양파
* 껍질을 안깐채로 바람숭숭 부는 곳에 둬야 가장 오래 보관된다.

# 마늘
* 껍질을 안깐채로 바람숭숭 부는 곳에 둬야 가장 오래 보관된다.

# 빵
* 1도에서 5도에 있으면(냉장고) 썩기 시작한다.(냉동고에 넣어서 실온에서 녹인 다음 먹어야 한다.)
...
# 라면
* 팜유(말레이시아, 인도네시아에서 나오는 기름 야자); 포화 지방산이 많다. 45%(웬만한 유의 2-3배)
* 모든지 가루로 만들면 인슐린 올라간다(밀가루, 설탕처럼 소화 흡수가 빨리 되는 음식은 먹자마자 우리의 혈관으로 들어간다.); 라면 스프
* 대부분의 라면에는 의외로 MSG(L-글루타민산나트륨)이 들어가있지 않다.
	* MSG는 사탕수수 뿐만아니라 소고기, 멸치, 무, 다시마표고보섯에 들어가있다.
* 한 포장지안에 많은 양의 면발을 담을 수 있고 더불어 면발 부서짐도 예방할 수 있기 때문에 면발이 꼬불꼬불하다


# 인슐린
* 모든지 가루로 만들면 인슐린 올라간다(밀가루, 설탕처럼 소화 흡수가 빨리 되는 음식은 먹자마자 우리의 혈관으로 들어간다.)

# MSG
* MSG를 안쓰려면 정말 복잡하게 단백질 아미노산을 쪼개고 분해시키고 해야 맛있는 성분으로 바뀌게끔 잘 조리를 하고 그걸 잘 배합하는 과정이 필요하다.
* MSG는 단백질의 기본구성인 아미노산 중에 하나일 뿐이다. 아미노산 글루탐산에 소금에 많은 나트륨을 붙혀놓은 것 뿐이다.
* MSG는 사탕수수 뿐만아니라 표고버섯, 돼지고기, 소고기, 멸치, 무, 다시마표고보섯에 들어가있다.
	* 저렴한 사탕수수에서 글루탐산이라는 아미노사만 추출하여 만든 것이 MSG
		* 사탕수수에서 추출해서 꺠끗하게 정제해서 농축해서 나트륨을 끼워 고체화한것 뿐
	* 사탕수수에서 추출한 MSG를 먹는 것과 표고버섯을 먹는 것은 다르다는 차이 그것 뿐 

# 유제품
## 버터, 마가린
* 버터: 지방 80%이상 + 수분 18% 이하; 우유에 있는 지방맨 빼낸 덩어리; 버터를 만들 땐 생크림과 우유 사용; 생크림은 우유에서 지방성분이 많으 부분만 뽑은 것; 생크림으로 버터를 만든 경우도 있음
	* 버터는 우유에 있는 지방을 간단한 공정을 거쳐서 분리해내서 고체화시킨것
* 미개봉 시 6개월 보관; 개봉후 냉장보관 1개월

## 마가린
* 프랑스 요리엔 버터가 자주 들어가는데 나폴레옹 3세가 버터 대신 먹을 수 있는 유통기한 길고 싼 버터를 화학자에게 요청
	* 소소기기름에 우유를 조금 넣어서 만듬; 하지만 지금은 식물성 기름으로 만들고 있음
	* 포화지방 많은 싸구려 식물성 기름, 팜유나 식물성 기름들에 수소경화법을 사용해서 고체화; 실온에서 액체로 존재하는 식물성 기름에 여러가지 첨가물과 향을 넣고 수소로 고체화
	* 올리브유같은 식물성 기름은 뜨겁게 가열하면 트랜스지방이 생긴다. 올리브유처럼 발연점이 낮은 불포화지방산이 많은 기름일수록 고온에서 가열을 하면 산패(기름이 썩는다는 의미); 반면에 동물성 지방의 기름은 가열해도 산패가 생기지 않는다.
	* 요새는 마가린을 가공하는 과정에서 트랜스지방 0g으로 만드는 기술 생김; 가열하면 생김
* 미개봉시에 1년 보관

# 액상과당
* 거의 모든 음료수에 들어가는 것; 옥수수로 만듬;
* 설탕의 반값 가경에 설탕의 단맛의 140% 
* 액체 상태라 과당, 포도당, 맥아당이 분리
* 설탕에 비해 식욕억제가 안된다(제로칼로리에 많이 들어간다.)
	* 오히려 혀는 단맛이 계속 나는데 식욕억제호르몬은 안나오니 계속해서 더 단맛이나 음식을 찾게 된다.

## 사카린
* 설탕의 300배의 단맛을 내는 감미료(뉴슈가)

## 설탕
* 사탕수수를 공장에서 뽀갠 후 쭉 짜내서 사탕수수즙을 만들어낸다. 그리고 여러가지 정제와 건조의 공정을 거쳐 갈아내면 설탕  
* 고체 상태여서 과당과 포도당이 결합 
* 설탕을 먹으면 식용억제 호로몬이 나온다(밥먹기전 단거 먹으면 식욕 감퇴)

# 김치
* 김치를 담구고 6개월이 지나면 유산균이 매일 감소한다. 그래서 고작 김치에는 식이섬유와 소금만 대부분 남게된다.

# 치킨
* 닭껍질이 닭한마리 지방의 90%를 차지한다.(닭 조리 전에 제거 필요)

# 음료

## 게토레이
* 플로리다 미식 축구의 경기력 향상을 위해 만듬; 생리식염수에 단맛과 여러가지 과일 첨가; 이후 9승 2패로 시즌 성적 향상
	* 체액이 부족할 때 먹으면 좋은 생리식염수와 같은 원리

## 포카리
* 그레이프후르츠농축과즙(이스라엘산 - 남아공) 첨가

# 채소
## 오이
* 인간은 PAV와 AVI 두가지 형으로 나뉘는데 PAV 유전자를 가진 사람은 쓴맛을 강하게 느낀다. 수박, 멜론, 참외에도 오이와 비슷한 쓴맛을 느낀다(100배에서 1000배까지)
	* 오이의 쿠쿠르비타신이라는 특유의 맛을 내는 물질 