...
```java
문 6. 직원(사번, 이름, 입사년도, 부서)테이블에 대한 SQL
문 중 문법적으로 옳은 것은?
1 SELECT COUNT(부서) FROM 직원 GROUP 부서;
2 SELECT * FROM 직원 WHERE 입사년도 IS NULL;
3 SELECT 이름, 입사년도 FROM 직원 WHERE 이름
=‘최%’;
4 SELECT 이름, 부서 FROM 직원 WHERE 입사년도
=(2014,2015);


답 2
2 옳은 표현이다. 입사년도가 기본키라는 말도 없으므로 NULL 값
을 가질 수 있다. NULL 값을 검색할 땐 ‘=’ 기호를 쓰지 않고,
‘IS NULL’ 표현을 쓴다.
입사년도 값이 비어있는 튜플을 출력한다.
<오답 체크> 1 GROUP 부서(Х) -> GROUP BY 부서(○)
-> SELECT COUNT(부서) FROM 직원 GROUP BY 부서;
튜플들을 부서가 같은 것끼리 그룹으로 묶은 뒤, 각 부서별로 튜
플 개수를 출력한다.
3 이름 = ‘최%’(Х) -> 이름 LIKE ‘최%’(○)
문자열를 찾을 땐 ‘=’ 기호가 아닌 ‘LIKE’ 표현을 쓴다.
‘%’는 문자열을 찾고, ‘_’는 한 글자만 찾는다.
-> SELECT 이름,입사년도 FROM 직원 WHERE 이름 LIKE ‘최%’
이름이 ‘최’로 시작하는 튜플의 이름과 입사년도를 출력한다.
4 입사년도 = (2014, 2015)(Х)
-> 입사년도=2014 OR 입사년도=2015(○)
-> 입사년도 IN(2014, 2015) (○)
둘 이상의 값 중 하나에 맞는 것을 찾을 땐 ‘OR’ 조건식으로 따
로 쓰거나, IN 구문으로 묶는다.
-> SELECT 이름,부서 FROM 직원

WHERE 입사년도 IN(2014, 2015);
입사년도가 2014년이나 2015년인 튜플을 찾아 출력한다.
