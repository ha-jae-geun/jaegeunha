# 문제
```java
스키마 R(A, B, C, D)와 함수적 종속{A->B, A->C}을 가질 때 다음 중 BCNF 정규형은? 4번
①	S(A, B, C, D)	 	②	S(A, B)와 T(A, C, D)
③	S(A, C)와 T(A, B, D)		④	S(A, B, C)와 T(A, D)

∙ A가 결정자가 되어 B를 결정하고 C를 결정하므로 A, B, C를 묶어서 하나의 테이블 S를 
구성하면 부분함수적 종속, 이행적 함수적 종속이 발생하지 않으므로 3정규형을 만족합니다. 
또한 A는 후보키가 되므로 모든 결정자는 후보키이 어야 한다는 BCNF 정규형까지 만족하게 됩니다.
∙ 하지만, D의 경우 어느 속성에도 종속되지 않아 분리를 해주어야 하는데 릴레이션 S와 연결되어야 
하므로 릴레이션 S의 기본키에 해당하는 A를 추가하여 T(A, D)를 구성합니다.


데이터베이스 관리 시스템(database management system)을 구축함으로써 생기는 이점만을 모두 고른 것은? 1번
ㄱ. 응용 소프트웨어가 데이터베이스에 관한 세부 사항에 자세히 관련할 필요가 없어져서 응용 소프트웨어 설계가 단순화될 수 있다.
...
∙ ㄷ 데이터베이스는 데이터의 논리적 구조나 물리적 구조가 변경되더라도 
  응용 프로그램이 영향 받지 않는 데이터 독립성(data independency)을 제공
∙ ㄹ 응용 소프트웨어는 데이터베이스 관리 시스템(DBMS)를 통하여 데이터베이스에 접근


15. 정규화의 목적으로 옳지 않은 것은? 2번
① 어떠한 릴레이션이라도 데이터베이스 내에서 표현 가능하
게 만든다.
...
# 반 정규화
* [반정규화](https://www.youtube.com/watch?v=SS6H2whbfwc&list=PLyQR2NzLKOCb-i7xCOQVU_yomhvHRteDy&index=17&t=0s)
* 성능향상을 위해 정규화를 포기하는것
* 데이터 무결성이 보장되지 않으므로 제한적으로 사용해야 한다.

```java
반정규화(De-normalization, 비정규화)
반정규화는 정규화된 엔티티, 속성, 관계를 시스템의 성능 향상 및 개발과 운영의 단순화를 위해 중복 
통합, 분리 등을 수행하는 데이터 모델링 기법 중 하나이다. 디스크 I/O 량이 많아서 조회 시 성능이 저하되거나,
테이블끼리의 경로가 너무 멀어 조인으로 인한 성능 저하가 예상되거나, 칼럼을 계산하여 조회할 때 성능이
저하될 것이 예상되는 경우 반정규화를 수행하게 된다. 일반적으로 조회에 대한 처리 성능이 중요하다고 판단될 때 
부분적으로 반정규화를 고려하게 된다.
...
3. 이력테이블 추가
 * 마스터 테이블에 존재하는 row를 트랜잭션 발생 지점에 따라 복사해두는 테이블 추가
